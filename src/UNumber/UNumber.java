...
public class UNumber implements Comparable<UNumber> {
    ...
    /**********
     * Returns the number of digits in the input. Required for the calculator.
     */
    public int size() {
        return d.length;
    }
...
//        System.out.println("d len: " + d.length);
        return this.d.length == 0 || this.d[0] == 0;
    }
     ...
     */
    public String displayBigNumber() {
        String str = "";
...
//        System.out.println(str);
    ...
    public UNumber sqrt() {
        if (this.isZero())
            return new UNumber(this);

        // We use Newton's method.
        int numSignificantDigits = 40;
        UNumber theValue =  new UNumber(this, numSignificantDigits);

        UNumber two = new UNumber(2.0, numSignificantDigits);

        UNumber newGuess = new UNumber(theValue, numSignificantDigits);				// Compute the estimate
        newGuess.div(two);
...
//    private int getNumDigits(String s) {
//        // If the string contains an E, then discard everything that's after the E, including the E.
//        int eIndex = s.indexOf('E');
//        if (eIndex != -1)
//            s = s.substring(0, eIndex);
//
//        // Now look for the decimal point.
//        int dIndex = s.indexOf('.');
//        if (dIndex != -1)
//            s = s.substring(dIndex + 1);
//
//        // Now we just have a whole number. Return its length.
//        return s.length();
//    }
//
//    private String getTruncatedNumber(String s, int n) {
//        int eIndex = s.indexOf('E');
//        boolean containsE = eIndex != -1;
//        String preE = "", postE = "", preD = "", postD = "";
//
//        if (containsE) {
...
//
//        int dIndex = s.indexOf('.');
//        boolean containsD = dIndex != -1;
//
//        if (containsD) {
//            String[] pieces = preE.split("\\.");
//            preD = pieces[0];
//            postD = pieces[1];
//        } else
//            postD = preE;
//
//        postD = postD.substring(0, n);
//
